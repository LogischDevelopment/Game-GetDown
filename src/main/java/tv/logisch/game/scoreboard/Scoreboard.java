...
//        setScore("Logisch.tv", 0);
    ...
    public void update() {
        ...
        if(state.equals(GameState.RUNNING) || state.equals(GameState.FINISHED)) {
            ...
            for (Player p : top) {
                ...
                topNames.add("§f" + name + " §8(§f" + p.getLocation().getBlockY() + "§8)");
            }
            ...
            setScore("§3", score++);
            setScore("§f" + this.player.getName() + " §8(§f" + this.player.getLocation().getBlockY() + "§8)", score++);
